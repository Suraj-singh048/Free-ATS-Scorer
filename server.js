  ...
  try {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      try {
        ...
        // NEW: Process ONE resume at a time for AI analysis
        ...
        try {
          ...
          // NEW: Send file directly to Gemini AI (no text extraction)
...
app.listen(PORT, () => {
  ...
  console.log(`📊 API endpoint: http://localhost:${PORT}/api/matcher`);
  console.log(`🤖 AI Provider: Google Gemini 2.5 Flash`);
  console.log(`🔧 Environment: ${process.env.NODE_ENV || 'development'}`);
  console.log(`🔑 API Key configured: ${process.env.GEMINI_API_KEY ? 'Yes ✓' : 'No ✗'}`);
  if (!process.env.GEMINI_API_KEY) {
    console.log(`⚠️  WARNING: GEMINI_API_KEY not found. Please add it to .env file`);
  }
});
